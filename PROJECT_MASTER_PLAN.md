...
### P2. Phase 1.5 자동 탐지 및 GPU 최적화

목표:

```text
YOLO11n scoreboard/overlay detection
GPU PaddleOCR batch inference
NVIDIA DALI 또는 ffmpeg 최적화 검토
Dockerfile.gpu 분리
```
...
7. `src/ocr/run_ocr.py` 구현
8. `src/ocr/clean_ocr.py` 구현
9. `src/ocr/smoothing.py` 구현
10. `src/evaluation/metrics.py` 구현
11. `outputs/reports/phase1_goal_recall_table.csv` 생성
12. GUI에 `OCR 실험 실행` 버튼 추가
